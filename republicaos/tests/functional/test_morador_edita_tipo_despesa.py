...
class TestMoradorCadastraTipoDespesa(TestController):
    def test_um(self):
        ...
        url_ = url(controller='tipo_despesa', action='edit', republica_id='1', id='1')
        ...
        assert 'Editar Tipo de Despesa' in response
        
        
        # editar tipo de despesa inválida:
        ...
        assert ('info', 'Tipo de Despesa alterado: Internet') == response.session['flash'][-1]
        assert TipoDespesa.get_by(nome='Internet', republica_id='1').id == 1
